...
(
    ID        NUMBER AUTO_INCREMENT
        CONSTRAINT PK_MEMBER_DETAIL PRIMARY KEY,
...
CREATE TABLE TBL_RECOMMENDED_POLICY_LIST
(
    ID NUMBER AUTO_INCREMENT PRIMARY KEY,
    FIRST_POLICY_ID VARCHAR(30) NOT NULL,
    SECOND_POLICY_ID VARCHAR(30) NOT NULL,
    THIRD_POLICY_ID VARCHAR(30) NOT NULL,
...
DROP TABLE TBL_MEMBER_PLUS;



SELECT TPM.POLICY_ID, TPM.POLICY_NAME, TPE.MARRY_STATUS_CODE, TPE.AGE_LIMIT_FLAG

FROM TBL_POLICY_MASTER TPM
         INNER JOIN TBL_POLICY_ELIGIBILITY TPE ON TPM.POLICY_ID = TPE.POLICY_ID
WHERE
--     TPM.MAIN_CATEGORY = '일자리'
--         AND
    TPM.ADDRESS_CODE LIKE CONCAT('%', 51, '%')
        AND (TPE.AGE_LIMIT_FLAG = 'Y'
        OR (
            TPE.AGE_LIMIT_FLAG = 'N'
            AND '50' BETWEEN TPE.MIN_AGE AND TPE.MAX_AGE)
            )
