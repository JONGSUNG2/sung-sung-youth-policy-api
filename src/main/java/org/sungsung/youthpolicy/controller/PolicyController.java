...
import org.sungsung.youthpolicy.domain.dto.policy.PolicyRecommendDTO;
...
public class PolicyController {
    ...
    public String policyRecommendPage(Model model, Authentication user){

        PolicyRecommendDTO policyRecommendDTO = new PolicyRecommendDTO();
        model.addAttribute("PolicyRecommendDTO", policyRecommendDTO );
        model.addAttribute("mainCategories", MainCategory.values());
        model.addAttribute("regions", Region.values());
...
//        return "policy/recommendedList";
    ...
    public String joinMemberPlus(@ModelAttribute PolicyRecommendDTO policyRecommendDTO, Model model, Principal principal, RedirectAttributes redirectAttributes) {
        List<PolicyListResponseDTO> recommendList = policyService.policyRecommend(policyRecommendDTO);
        log.info("recommendList={}", recommendList);
        model.addAttribute("recommendedList", recommendList);
        return "policy/recommendedList";
    }
